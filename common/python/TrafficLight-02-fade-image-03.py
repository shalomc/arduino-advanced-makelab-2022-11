...
graf_title = 'G parabole ; Y reverse absolute ; R sine'
INTERACTIVE = False
...
def traffic_green_value(sensorValue, max_value = 255):
    # calculate parabole
    # must use long type in arduino code
    valueFunction = (sensorValue-512)**2
    valueComputed = arduino_map(valueFunction, 0, 1024*256, 0, max_value) * (sensorValue < 512)
    return valueComputed # arduino_map(valueComputed, 0,1000,0,max_value)
# ------------------------------------------------------
def traffic_yellow_value(sensorValue, max_value = 255):
    # calculate reverse absolute
    valueComputed = max_value - int(abs(sensorValue - 512) / (512 / max_value))
